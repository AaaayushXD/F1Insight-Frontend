import { Flag, Github, Twitter, Instagram } from "lucide-react"
import { Link } from "react-router-dom"

export function Footer() {
  return (
    <footer className="relative border-t border-f1-graphite bg-[#0B0D10] pt-16 pb-8">
      {/* Red Accent Top Line */}
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 gap-12 md:grid-cols-4">
          ...
          <div className="md:col-span-2">
            ...
            <p className="max-w-xs text-sm text-f1-steel leading-relaxed">
              The absolute pinnacle of F1 analytics. Engineering precision, data-driven decisions, and predictive intelligence.
            </p>
          </div>

          {/* Platform Links */}
          <div>
            <h4 className="font-orbitron text-xs font-bold tracking-[0.2em] text-white mb-6 uppercase">Platform</h4>
            <ul className="space-y-4">
              {['Schedule', 'Drivers', 'Constructors', 'Circuits'].map((item) => (
                <li key={item}>
                  <Link 
                    to={`/${item.toLowerCase()}`} 
                  ...
                  >
                    {item}
                    <span className="absolute -bottom-1 left-0 h-[1px] w-0 bg-f1-red transition-all duration-300 group-hover:w-full" />
                  </Link>
                </li>
              ))}
            </ul>
          ...
          {/* Social / Connect */}
          <div>
            <h4 className="font-orbitron text-xs font-bold tracking-[0.2em] text-white mb-6 uppercase">Connect</h4>
        ...
        <div className="mt-16 pt-8 border-t border-f1-graphite flex flex-col items-center justify-between gap-4 md:flex-row">
          <p className="text-xs text-f1-steel">
            &copy; 2026 F1INSIGHT. ENGINEERED BY FANS FOR ENTHUSIASTS.
          </p>
          <div className="flex gap-8 text-[10px] font-bold uppercase tracking-widest text-f1-steel">
            <a href="#" className="hover:text-f1-red transition-colors">Terms of Service</a>
